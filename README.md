...
On Debian/Ubuntu, executing the following command should suffice:
...
Several standard packages are needed to use the toolchain.  
On Debian/Ubuntu, executing the following command should suffice:

    $ sudo apt-get install libmpc3 libmpfr6 libgmp10 zlib1g libzstd1 libexpat1 libisl23

On Fedora/CentOS/RHEL OS, executing the following command should suffice:

    $ sudo yum install libmpc mpfr gmp zlib libzstd expat isl
    $ sudo yum install cross-gcc-common cross-binutils-common # optionals
...
run the following command:
    ...
    ./configure --prefix=/opt/riscv --with-arch=rv32imcxpulpv2 --with-abi=ilp32 --enable-multilib --build-genbin
    make
